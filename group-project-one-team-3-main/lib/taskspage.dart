...
class _TasksPageState extends State<TasksPage> {
            ...
            child: Container(
              ...
              child: Center(
                child: Container(
                  ...
                  child: StreamBuilder<List<Task>>( /* StreamBuilder will listen to the stream of tasks through tasksStream */
                    stream: tasksStream,
                    /* snapshot to check if we are able to access firebase data successfully, which is the result of a future. */
                    builder: (context, snapshot) {
                      /* waiting for successful connection to firebase db */
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        return CircularProgressIndicator();
                        /* if an error has occurred connecting to firebase */ 
                      } else if (snapshot.hasError) {
                        return Text('Error: ${snapshot.error}');
                        /* if database is empty for current user */
                      } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                        return Text('No tasks found.');
                        /* if successful connection to firebase database for current user logged in. */
                      } else {
                        final tasks = snapshot.data!; /* tasks is non null as we passed error handling and checking for empty tasks */
                        return ListView(
                          /* scrolling functionality added to parent container where each task will be stored */
                          controller: _scrollController,
                          /* iterating through each task found and applying the styling below for each task tab */
                          children: tasks.map((task) {
                            return Column(
                              children: [
                          Container(
                            /* styling our todo tabs */
                            decoration: BoxDecoration(
                              ...
                              /* boxShadow styling for our todo tabs */
                              boxShadow: [
                                BoxShadow(
                            ...
                            /* Add padding inside the TodoBox */
                              child: Padding(
  ...
  /* returning a stream of data asyncronously, awaiting real-time data updates to the database as the user can delete and add tasks. */
